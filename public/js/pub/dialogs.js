define(["jquery", "Module"], function($, M) {
    ...
    Dialog.M = function (opts) {
        ...
        //弹窗id
        d.id = opts.id || "layout-diglog-wrapper";

        //弹窗外部样式
        d.cls = opts.cls || "";
        d.css = opts.css || {};
        d.diglogWrapper = '<div id="' + d.id + '" class="layout-diglog-wrapper"></div>';
        ...
        d.diginfo = M.ParseTpl(d.msgbody, $.extend({},
            d.tpl, {
                close: d.close,
                msg: d.msg
            }));
    ...
    Dialog.M.prototype = {
        ...
        Init: function() { // 弹窗
            ...
            d.closeBtn.bind("click", function() {
                ...
                if (d.closeAll) {
                    $(".layout-diglog-wrapper, .layout-diglog-overlay").remove();
                    $.each(d.cls.split(" "), function(i, j) {
                        $("." + j).remove();
                    })
                }
                if ($(".layout-diglog-overlay").length !== 0) {
                    if (d.$d.prop("overlay") == "yes") {
                        $(".layout-diglog-overlay").remove();
                    }
                }
                //清除掉全局window事件。
        ...
        Overlay: function(opts) {
            var d = this;
            if (d.overlay && $(".layout-diglog-overlay").length == 0) {
                var css = {
                    "background": "#000",
                    "width": "100%",
                    "height": d.docHeight,
                ...
                var _overlay = $('<div class="layout-diglog-overlay"></div>');
        ...
        autoClose: function(times, auto) {
            ...
            if (auto) {
                M.getTime.countTimer[d.id] = window.setTimeout(function() {
